...
OutputBaseFilename=EveConnector-Setup-v5.5
...
[Dirs]
Name: "{app}\eve-connector-win"; Components: eveconnector
Name: "{app}\eve-connector"; Components: eveconnector
...
Source: "eve-connector-win\server.js"; DestDir: "{app}\eve-connector-win\"; Flags: ignoreversion; Components: eveconnector; AfterInstall: InstallEveConnectorService
; Boca printer
...
Name: "winusb"; Description: "Pilotes WinUSB pour les p�riph�riques"; Types: full custom
...
[UninstallDelete]
Type: filesandordirs; Name: "{app}/eve-connector"; Components: eveconnector
...
begin
  ...
  begin
    if not WizardSilent then 
      MsgBox(errorMessage + '%n%nAbandon de l''installation de EveConnector.', mbCriticalError, MB_OK);
...
begin
  ...
  if not res then begin
    errorMessage := 'L''installation de Node.js a �chou�: ' + IntToStr(errorCode);
    Log(errorMessage);
    aborted := True;
    exit;
  end;
...
begin
  ...
  res := ShellExec('runas', 'msiexec.exe',
    ...
    '', showWindow, ewWaitUntilTerminated, errorCode);
  if not res then begin
    errorMessage := 'La d�sinstallation de Node.js a �chou�: ' + IntToStr(errorCode);
    Log(errorMessage);
  end;
end;
...
procedure InstallEveConnectorService();
var
  errorCode: Integer;
  res: Boolean;
begin
  Log('InstallEveConnectorService() called');
  if aborted then exit;
  ...
  if not res then 
  begin
    errorMessage := 'La configuration du service a �chou�: ' + IntToStr(errorCode) + ' / ' + ExpandConstant('{app}\eve-connector-win');
    Log(errorMessage);
    aborted := True;
    exit;
  end;
